...
def _extract_port_list(mod_content: str) -> str:
    """Return the raw port list string (parentheses included) from a module header.

    Finds the first ``(`` in *mod_content* and walks forward, counting nesting
    depth, until the matching ``)`` is reached.  Returns the substring
    ``(<ports>)`` or an empty string when no opening paren is found.
    """
    start = mod_content.find("(")
    if start == -1:
        return ""
    depth = 0
    for i, ch in enumerate(mod_content[start:], start):
        if ch == "(":
            depth += 1
        elif ch == ")":
            depth -= 1
            if depth == 0:
                return mod_content[start : i + 1]
    return ""
...
def split_systemverilog(input_file: str | Path, output_dir: str | Path = "src/rtl") -> None:
    ...
    for mod_content, mod_name in module_matches:
        ...
        if is_top:
            ...
            port_section = _extract_port_list("\n".join(new_lines))
            wrapper_lines = [
                "",
                "// " + "=" * 77,
                ...
                "// " + "=" * 77,
                f"module {top_alias_name}",
                "import cgra_pkg::*;",
                port_section + ";",
                f"  {mod_name} u_top (.*);",
                f"endmodule : {top_alias_name}",
            ]
            mod_content_out = "\n".join(alias_lines + new_lines + wrapper_lines)
        else:
            mod_content_out = "\n".join(new_lines)
